...
%%%%%%%%%%%%%%%%%%%%%%
% 2.1 - LOAD THE VIDEO
video3 = VideoReader('3.mp4');
frames3 = read(video3, [1 Inf]);
[col, row, c, num_frames] = size(frames3);
% size(frames3) >> 1280y 720x 3c, 454f

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 2.3 - SELECT APPROXIMATELY 60 FRAMES
% NOTE: I CHANGED THE ORDER OF 2.2 AND 2.3 (DISCUSSED IN PAPER WHY)
short3 = frames3(:,:,:,1:7:end);
num_short = size(short3, 4);
% num_short >> 65f

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 2.2 - CONVERT FRAMES TO GRAYSCALE
for f = 1:num_short
    grays3(:,:,f) = rgb2gray(short3(:,:,:,f));
end

%%%%%%%%%%%%%%%%%%%%%%%%%
% 2.4 - SELECT A TEMPLATE
first_frame = grays3(:,:,1);

% water bottle
T1 = 80;
template1 = imcrop(first_frame, [240 620 T1-1 T1-1]);

% sketchbook
T2 = 100;
template2 = imcrop(first_frame, [430 590 T2-1 T2-1]);

% boxing gloves
T3 = 90;
template3 = imcrop(first_frame, [122 518 T3-1 T3-1]);
...
W1 = 600;
for i = 1:num_short
    window1(:,:,i) = imcrop(grays3(:,:,i), [60 425 W1-1 W1-1]);
end

% sketchbook
W2 = 500;
for i = 1:num_short
    window2(:,:,i) = imcrop(grays3(:,:,i), [300 400 W2-1 W2-1]);
end

% boxing gloves
W3 = 300;
for i = 1:num_short
    window3(:,:,i) = imcrop(grays3(:,:,i), [60 425 W3-1 W3-1]);
end
...
figure()
imshow(first_frame)
imrect(gca, [240 620 T1-1 T1-1]);
imrect(gca, [430 590 T2-1 T2-1]);
imrect(gca, [122 518 T3-1 T3-1]);
...
nexttile
P1 = SyntheticAperture(short3, x1, y1);
imshow(P1)
title('Synthethic Aperture focused on WATER BOTTLE')
nexttile
P2 = SyntheticAperture(short3, x2, y2);
imshow(P2)
title('Synthethic Aperture focused on SKETCHBOOK')
nexttile
P3 = SyntheticAperture(short3, x3, y3);
imshow(P3)
